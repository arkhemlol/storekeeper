...
const GameView = Marionette.View.extend({
  ...
  initializeListeners() {
    ...
    this.listenTo(this._game, 'level:completed', () => {
      ...
      const levelNumber = this._game.levelSet.levelNumber;
  ...
  browseLevelSet() {
    const $fileInput = $('<input />').attr('type', 'file').attr('accept', '.json,.sok');
  ...
  onKeyDown(event) {
    ...
    const direction = GameDirection.byKeyCode(event.which);
  ...
  onTouchStart(event) {
    ...
    const x = touch.clientX - $canvas.offset().left,
